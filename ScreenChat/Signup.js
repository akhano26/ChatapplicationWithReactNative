...
export default function Signup({navigation}){
  ...
  const onHandleSignup=()=>{
    console.log('punch gia')
    if(email!==""&&password!==""){
      createUserWithEmailAndPassword(auth,email,password)
      .then(()=>console.log("Signup Success"))
      .catch((error)=>Alert.alert("Something went wrong in Signup",error.message))
    }
  }
  // const onHandleLogin=()=>{
  //   if(email !=="" &&password!=""){
...
<View style={{marginTop:20,flexDirection:'row',alignItems:'center',alignSelf:'center'}}>
  <Text style={{color:'gray', fontWeight:600,fontSize:18}}>Already have Account? </Text>
  <TouchableOpacity>
    <Text style={{color:'#f57c00',fontWeight:'600',fontSize:18}} onPress={()=>navigation.navigate('Login')} >Login</Text>
  </TouchableOpacity>
</View>
</SafeAreaView>   
  </View>
